...
class TkinterFace:
  def __init__(self, root):
    ...
    self.face_parameters = {
      "look_target": {
        "current_value": np.array([400.0, 150.0, 1000.0]),
        "target_value": np.array([400.0, 150.0, 1000.0]),
        "max_speed": 20,
        "min_speed": 1,
        "speed_factor": 1.0
      },
      ...
      "face_position": {
        "current_value": np.array([400.0, 150.0, 0]),
        "target_value": np.array([400.0, 150.0, 0]),
        "max_speed": 20,
        "min_speed": 1,
        "speed_factor": 1.0,
      },
  ...
  def update(self):
    ...
    eye_scale = 25.0 * self.face_parameters["face_scale"]["current_value"]
    eye_x = 2.75
    eye_size = np.array([eye_scale * eye_x, eye_scale * eye_open_value])
    eye_half_size = eye_size / 2

    # Keep the pupil within the eye
    pupil_radius = 20.0 * self.face_parameters["face_scale"]["current_value"]
    ...
    eye_distance = 75.0 * self.face_parameters["face_scale"]["current_value"]
    ...
    mouth_min_frequency = 5.0 * self.face_parameters["face_scale"]["current_value"]
    mouth_max_frequency = 20.0 * self.face_parameters["face_scale"]["current_value"]
    ...
    mouth_amplitude = 50.0 * self.face_parameters["face_scale"]["current_value"] * self.face_parameters["mouth_open_factor"]["current_value"]
    mouth_sine = mouth_sine * mouth_amplitude

    # Calculate mouth position based on face position and mouth offset
    mouth_offset_pos = np.array([0, 150.0]) * self.face_parameters["face_scale"]["current_value"]
    mouth_center = np.array([0, 0])
    mouth_length = 200.0 * self.face_parameters["face_scale"]["current_value"]
